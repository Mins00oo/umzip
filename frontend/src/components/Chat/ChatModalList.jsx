...
export default function ChatModalList({
  toggleDropdown,
  setChatList,
  name,
  chat,
  ...
  const truncatedTitle = chat.length > 9 ? chat.slice(0, 9) + "..." : chat;
  useEffect(() => {
    scrollToBottom();
  ...
  useEffect(() => {
    ...
    if (storedUserInfo) {
      ...
      setUserInfo({
        ...
        profileImage: parsedInfo.profileImage,
        id: parsedInfo.id,
      });
    } else {
      navigate("/login");
  ...
  const handleBuy = async (e) => {
    ...
    if (userConfirmed) {
      ...
      if (answer) {
        stopSocketCommunication(), setOpenModal(false);
        setTalkHistory([]);
        setuserinput("");
          ...
          >
            ...
            >
              ...
              >
                {tradeChat && (
                  <motion.div
                    onClick={() => navigate(`/trade/${tradeChat.tradeId}`)}
                    whileHover={{
                      y: -5,
                      cursor: "pointer",
                  ...
                  >
                    ...
                    >
                      <div className="d-flex  justify-content-between">
                        <h5 className="m-0 ">
                          제목 : {tradeChat.tradeItemTitle}
                        </h5>
                        {tradeChat.memberId !== userinfo.id && (
                          <button onClick={handleBuy} className="btn btn-light">
                            구매확정
                          </button>
                        )}
                      </div>
                      <h5 className="m-0 ">
                        가격 : {tradeChat.tradeItemPrice}원
                      </h5>
